...
class RxChannel {
  ...
  deleteQueue(queue: string, options?: Options.DeleteQueue): Observable<DeleteQueueReply> {
    return from(this.channel.deleteQueue(queue, options)).pipe(
      map((reply: Replies.AssertQueue) => new DeleteQueueReply(this, reply)));
  }
  ...
  purgeQueue(queue: string): Observable<PurgeQueueReply> {
    return from(this.channel.purgeQueue(queue)).pipe(
      map((reply: Replies.AssertQueue) => new PurgeQueueReply(this, reply)));
  }
  ...
  unbindQueue(queue: string, source: string, pattern: string, args?: any): Observable<EmptyReply> {
    return of(this.channel.unbindQueue(queue, source, pattern, args)).pipe(
      map(() => new EmptyReply(this)));
  }
  ...
  bindExchange(destination: string, source: string, pattern: string, args?: any): Observable<EmptyReply> {
    return from(this.channel.bindExchange(destination, source, pattern, args)).pipe(
      map(() => new EmptyReply(this)));
  }
  ...
  unbindExchange(destination: string, source: string, pattern: string, args?: any): Observable<EmptyReply> {
    return from(this.channel.unbindExchange(destination, source, pattern, args)).pipe(
      map(() => new EmptyReply(this)));
  }
  ...
  public consume(queue: string, options?: Options.Consume): Observable<RxMessage> {
    return <Observable<RxMessage>> Observable.create(observer => {
      let tag: string;
      let close$ = fromEvent(<any> this.channel, 'close');
